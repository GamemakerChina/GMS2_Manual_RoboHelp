...
you can right click
![](/assets/Images/Icons/Icon_RMB.png) on it in the
Asset Browser to open the context menu, which will permit you to create
**child rooms** (see the page on
...
![](/assets/Images/Asset_Editors/Editor_Room_RoomManager.png)  
The room editor is itself a [workspace](../Introduction/Workspaces)
and as such you can click
![](/assets/Images/Icons/Icon_LMB.png) on the tab
and drag it off of the main window into a new window of its own -
perhaps in another display, for example. You can also place it back into
the main window by dragging the tab to the top of the IDE and releasing
the mouse button. The user interface for the room editor is simple to
navigate and split in various discrete sections. Those parts of the
editor that are docked - the **room properties** and the **layer
...
![](/assets/Images/Asset_Editors/Editor_Sequences_Guides.png)  
...
|                                                                    |                   |                                                                                                                                                                          |
|--------------------------------------------------------------------|-------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ![](/assets/Images/Icons/Icon_Instance.png)   | **Instances**     | This layer is for placing [instances](../Quick_Start_Guide/Objects_And_Instances) that will be used in the room.                                                     |
| ![](/assets/Images/Icons/Icon_Tilemap.png)    | **Tile Maps**     | This layer is for placing tiles from [tile sets](../Quick_Start_Guide/Creating_Tile_Sets) as tile map s .                                                            |
| ![](/assets/Images/Icons/Icon_Paths.png)      | **Paths**         | This layer is for showing and adding [paths](Paths) in the room.                                                                                                     |
| ![](/assets/Images/Icons/Icon_Assets.png)     | **Assets**        | This layer is for visual assets to be added to the room, like [sprites](../Quick_Start_Guide/Creating_Sprites) and [sequences](../Quick_Start_Guide/Sequences) . |
| ![](/assets/Images/Icons/Icon_FX_Layer.png)   | **Filter/Effect** | This type of layer defines a [filter/effect](Room_Properties/Filters_and_Effects) that is applied to all layers below it.                                            |
| ![](/assets/Images/Icons/Icon_Background.png) | **Background**    | This type of layer defines a background, which is essentially a single colour or sprite image that may be tiled over the whole room.                                     |

You can also create a layer folder using the folder button
![](/assets/Images/Icons/Icon_Folder.png) where you
can group selected layers together, as well as delete the selected
layers with the delete button
...
![](/assets/Images/Icons/Icon_Eye.png) beside each
of the layers, or you can click the one at the very top to
...
that layers which are flagged as invisible will **not** be shown when
the game is run. [ Layer Properties Layer Properties ](#) The room layer
properties window will change depending on the currently selected layer
in the layer editor.  
![](/assets/Images/Asset_Editors/Editor_Room_LayerProperties.png)  
...
-   ![](/assets/Images/Icons/Icon_CanvasGrid.png)
    **Toggle Canvas Grid** : This will toggle on/off the Room Editor
    ...
    ![](/assets/Images/Asset_Editors/Editor_RoomGridControls.png)  
    ...
    ![](/assets/Images/Icons/Icon_Shift.png) + "G"
    to toggle the grid visibility and grid snapping respectively.
-     
    ...
    **Canvas Zoom Controls** : These buttons control the current canvas
    ...
    make the entire room canvas fit within the current editor workspace
    ...
    set the room canvas to be 1:1 with the room settings.
-   **![](/assets/Images/Icons/Icon_CanvasViews.png)
...
-   **![](/assets/Images/Icons/Icon_PlayRoom.png)
...
-   **![](/assets/Images/Icons/Icon_CanvasSelect.png)
...
middle mouse button
![](/assets/Images/Icons/Icon_MMB.png) down and
moving the mouse around or using
![](/assets/Images/Icons/Icon_Space.png) +
![](/assets/Images/Icons/Icon_LMB.png) while
moving. When creating asset layers or instance layers, you can place the
asset or instance by simply dragging it from the Asset Browser and then
dropping it where you want it to be positioned. Alternatively you can
select an asset or instance from the Asset Browser and then press and
hold ![](/assets/Images/Icons/Icon_Alt.png) to
preview the resource "in-situ", and if you additionally click the left
...
![](/assets/Images/Icons/Icon_Ctrl.png) /
![](/assets/Images/Icons/Icon_Cmd.png) to have the
instance follow the mouse precisely and not snap (releasing
![](/assets/Images/Icons/Icon_Ctrl.png) /
![](/assets/Images/Icons/Icon_Cmd.png) will permit
snapping again). For paths you can create a new path layer and path
resource right from within the Room Editor- or create a new path
resource and then drag it into the main editor window as you would for
an instance - and then edit the path and its connections in the editor
window too. For tile sets, you can "paint" them in from the tile set
editor. Note that you can select and move or delete multiple assets from
the same layer by holding down
![](/assets/Images/Icons/Icon_Ctrl.png) /
![](/assets/Images/Icons/Icon_Cmd.png) +
![](/assets/Images/Icons/Icon_LMB.png) on the ones
you want, and most resources in the room editor will permit you to
rotate and scale them (singly or as a group) by clicking and holding
![](/assets/Images/Icons/Icon_LMB.png) along the
outside edges or corners and dragging the mouse. You can also double
click ![](/assets/Images/Icons/Icon_LMB.png) on any
single asset (tiles, instances, sprites) to have the editor pan to it
and open up the properties window for the asset selected. [ Status Bar
Status Bar ](#) The status bar is used to show you context specific
...
![](/assets/Images/Asset_Editors/Editor_RoomContextMenu.png)  
...
-   **Instance Creation Order** : This will open the **Instance Creation
    ...
    ![](/assets/Images/Asset_Editors/Editor_Room_CreationOrder.png)  
    ...
    click ![](/assets/Images/Icons/Icon_LMB.png)
...
-   **Tile Editing** : When working with a tile layer, this option will
    ...
    sub-menu:  
    ![](/assets/Images/Asset_Editors/Editor_Room_TileMenu.png)  
    When editing the tile layer you can select multiple tiles by
    clicking, holding down and dragging the left mouse button
    ![](/assets/Images/Icons/Icon_LMB.png) , or you
    can select and group individual tiles using
    ![](/assets/Images/Icons/Icon_Ctrl.png) /
    ![](/assets/Images/Icons/Icon_Cmd.png) +
    ![](/assets/Images/Icons/Icon_LMB.png) , or
    remove individual tiles from the selection using
    ![](/assets/Images/Icons/Icon_Alt.png) +
    ![](/assets/Images/Icons/Icon_LMB.png) . From
    this menu you can then clear the selection or invert it as well as
    cut, copy or paste the selection to another layer (or even the same
    layer but in another position). The final option will re-open the
    tile set editor docked window should you have closed it.
-   **Convert Image To Tile Map** : The Convert Image To Tile Map option
    ...
    **Image Import** tool:  
    ![](/assets/Images/Asset_Editors/Editor_Room_ConvertImage.png)  
    ...
    base image:  
    ![](/assets/Images/Asset_Editors/Editor_Room_TileExample.png)  
